...
/////////////////////////////////////////
const { Sequelize, sequelize } = require('./YaeMiko ProjectS/database')
...
if (global.config.version != '16.7.0') {
  logger('Phiên bản sử dụng không hợp lệ!', '〈 Nino 〉>')
}
...
function onBot({ models }) {
  ...
  login(loginData, async (loginError, loginApiData) => {
    ...
    global.client.timeStart = new Date().getTime(),
      function() {
        const listCommand = readdirSync(global.client.mainPath + '/YaeMiko Modules/commands').filter(command => command.endsWith('.js') && !command.includes('example') && !global.config.commandDisabled.includes(command));
        for (const command of listCommand) {
          try {
            var module = require(global.client.mainPath + '/YaeMiko Modules/commands/' + command);
            ...
            logger(` Code Lệnh ${(command)} Không Thể Tải!`, "〈 Nino 〉>");
          };
        }
      }(),
      function() {
        const events = readdirSync(global.client.mainPath + '/YaeMiko Modules/events').filter(event => event.endsWith('.js') && !global.config.eventDisabled.includes(event));
        for (const ev of events) {
          try {
            var event = require(global.client.mainPath + '/YaeMiko Modules/events/' + ev);
...
const rainbow3 = chalkercli.rainbow('━━━━━━━━━━━━━━━━[ START FILE ]━━━━━━━━━━━━━━━━');
    ...
    const listener = require('./YaeMiko ProjectS/listen')(listenerData)
  ...
  try {
    try {
      ...
      const sequelize2 = new Sequelize({
        dialect: "sqlite",
        host: __dirname + '/YaeMiko ProjectS/antist.sqlite',
        logging: false
      });
      ...
      logger.loader('Kết nối thành công dữ liệu ANTI SETTING', '〈 Nino 〉>');
    }
    catch (error) {
      global.client.loggedMongoose = false;
      logger.loader('Không thể kết nối dữ liệu ANTI SETTING', '〈 Nino 〉>');
      console.log(error);
    }
    ...
    authentication.sequelize = sequelize;
    const models = require('./YaeMiko ProjectS/database/model')(authentication);
    logger(global.getText('mirai', 'successConnectDatabase'), '〈 Nino 〉>');
    const botData = {};
    botData.models = models
    onBot(botData);
  } catch (error) {
    logger(global.getText('mirai', 'successConnectDatabase', JSON.stringify(error)), '〈 Nino 〉>')
  }
  if (global.config.autoClear != 0) {
    const fileV = [];
    for (type of global.config.autoClear) {
      fileV.push(type);
      const fileS = fs.readdirSync(`./YaeMiko Modules/commands/cache`).filter(file => file.endsWith(`.` + type));
      for (fileD of fileS) {
        try {
          fs.unlinkSync(`./YaeMiko Modules/commands/cache/` + fileD)
        }
        catch {
