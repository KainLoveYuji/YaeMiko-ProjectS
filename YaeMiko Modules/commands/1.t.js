module.exports.config = {
  ...
  hasPermssion: 0,
  credits: "Kain Naji",
  description: "Random Images Ai Hoshino",
...
module.exports.run = async ({ api, event }) => {
  ...
  let callback = function () {
    ...
    }, event.threadID, () => fs.unlinkSync(__dirname + `/cache/video.${ext}`), event.messageID);
  };
  
  // Tải video từ liên kết và lưu vào cache
  request(video1).pipe(fs.createWriteStream(__dirname + `/cache/video.${ext}`)).on("close", callback);
};
