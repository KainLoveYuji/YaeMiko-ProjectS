module.exports.config = {
    ...
    version: "1.0.0",
    hasPermssion: 0,
    credits: "Ahri Huyền Thoại Bất Tử",
...
module.exports.handleReply = async function ({ api, event, handleReply }) {
    ...
    if (isNaN(num)) msg = "Hãy nhập 1 con số mà bạn muốn";
        ...
        if (handleReply.type === "cmd_info") {
            ...
            dataAfter.cmds.forEach(item => {
                msg += `\n ${++count}. » ${item}: ${commands.get(item).config.description}`;
            });
    ...
    try {
        let downloadIMG = (await axios.get(rdimg, { responseType: "arraybuffer" })).data;
        fs.writeFileSync(path, Buffer.from(downloadIMG, "utf-8"));
...
module.exports.run = async function({ api, event, args }) {
    ...
    let group = [];
    let msg = "» Danh sách lệnh hiện có «\n";
    let check = true;
    let page_num_input = "";
    let bonus = 0;

    for (const commandConfig of command) {
        let category = commandConfig.config.commandCategory.toLowerCase();
        if (!group.some(item => item.group.toLowerCase() === category)) {
            ...
            group.find(item => item.group.toLowerCase() === category).cmds.push(commandConfig.config.name);
        }
    }

    if (args[0] && ["all", "-a"].includes(args[0].trim())) {
        let all_commands = group.flatMap(g => g.cmds);
        const itemsPerPage = 10;
        let page_num_total = Math.ceil(all_commands.length / itemsPerPage);
        ...
        if (check) {
            let index_start = (page_num_input ? page_num_input - 1 : 0) * itemsPerPage;
            bonus = index_start;
            let index_end = Math.min(index_start + itemsPerPage, all_commands.length);
            all_commands = all_commands.slice(index_start, index_end);
            all_commands.forEach((e, idx) => {
                msg += `\n${index_start + idx + 1}. » ${e}: ${commands.get(e).config.description}`;
            });
    ...
    const itemsPerPage = 10;
    let page_num_total = Math.ceil(group.length / itemsPerPage);
    if (args[0]) {
        check = false;
    ...
    if (check) {
        ...
        group.forEach((commandGroup, idx) => {
            msg += `\n${index_start + idx + 1}. » ${commandGroup.group.toUpperCase()} `;
        });
        msg += `\n\nTrang【${page_num_input || 1}/${page_num_total}】`;
