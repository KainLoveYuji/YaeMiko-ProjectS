...
function startBot(message) {
    ...
    const child = spawn("node", ["--trace-warnings", "--async-stack-traces", "YaeMikoandNino.js"], {
        cwd: __dirname,
        stdio: "inherit",
        shell: true
    });
...
function startBot(message) {
    ...
    child.on("close",async (codeExit) => {
      var x = 'codeExit'.replace('codeExit',codeExit);
        if (codeExit == 1) return startBot("Bot Yae Miko & Nino đang khởi động lại");
         else if (x.indexOf(2) == 0) {
           await new Promise(resolve => setTimeout(resolve, parseInt(x.replace(2,'')) * 1000));
                 startBot("Bot Yae Miko & Nino đang hoạt động");
       }
         else return; 
    });
...
setTimeout(async function () {
  ...
  await new Promise((data) => setTimeout(data, 500))
logger("Bot Yae Miko & Nino đang tải dữ liệu hệ thống", "〈 Nino 〉>")
